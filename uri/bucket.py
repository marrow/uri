...
class Bucket:
    ...
    def __init__(self, name, value="", sep="=", strict=False):
        ...
        if not value:
            if isinstance(name, str):
                if name.count(sep) > 1:
                    if strict:
                        raise ValueError(f"Multiple occurrences of separator {sep!r} in: '{name}'")
                    self.valid = False
    ...
    def __str__(self):
        ...
        iterator = (
            (quote_plus(i.encode("utf8")).replace("%3F", "?").replace("%2F", "/") for i in self) if self.valid else self
        )
